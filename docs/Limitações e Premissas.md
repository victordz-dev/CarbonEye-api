...
* **Piso de Metragem por Polígono:** Para garantir a compatibilidade com a resolução do satélite, o sistema impõe um limite mínimo de 1 hectare (10.000m²) por polígono. O sistema não impõe teto máximo de tamanho por triagem, desde que a cota global da conta não seja excedida.
* **Restrições de Cota do Usuário:** Para viabilizar a distribuição justa de recursos, cada conta de usuário possui uma cota máxima de 50 hectares (ha) acumulados. Adicionalmente, o usuário pode manter até 2 mapas sob o regime de monitoramento contínuo.
* **Irreversibilidade do Consumo Orbital:** O limite global fornecido pela API AgroMonitoring é absoluto. A exclusão de uma área do histórico não devolve a área (em hectares) processada ao saldo do limite da conta do usuário.
